...
def detect_smc_fast(df, i, window=50):
    ...
    for j in range(1, n-1):
        if l[j+1]>h[j-1] and (l[j+1]-h[j-1])>atr_val*0.25:
            fvgs.append({'type':'bull','top':l[j+1],'bot':h[j-1]})
        if h[j+1]<l[j-1] and (l[j-1]-h[j+1])>atr_val*0.25:
...
def check_signal(price, obs, fvgs, bos_dirs, row):
    ...
    ema50  = row['ema50']
    rsi    = row['rsi']
    vol_ok = row['volume'] > row['vol_ma'] * 1.15

    if not vol_ok: return None,0,0,0,0

    # LONG — tendencia + OB + FVG + BOS (las 3)
    if ema9 > ema21 > ema50 and 30 < rsi < 60:
        # OB: precio dentro o tocando el OB (zona extendida con ATR)
        bull_ob  = next((o for o in obs if o['type']=='bull' and
                         o['low']-atr*0.2 <= price <= o['high']+atr*0.2), None)
        bull_fvg = next((f for f in fvgs if f['type']=='bull' and
                         f['bot']-atr*0.1 <= price <= f['top']+atr*0.1), None)
        if bull_ob and bull_fvg and 'bull' in bos_dirs:
            sl = bull_ob['low'] - atr*0.5
            tp = price + (price-sl)*3
            return 'LONG', 100, price, sl, tp

    # SHORT — tendencia + OB + FVG + BOS (las 3)
    if ema9 < ema21 < ema50 and 40 < rsi < 70:
        bear_ob  = next((o for o in obs if o['type']=='bear' and
                         o['low']-atr*0.2 <= price <= o['high']+atr*0.2), None)
        bear_fvg = next((f for f in fvgs if f['type']=='bear' and
                         f['bot']-atr*0.1 <= price <= f['top']+atr*0.1), None)
...
all_results = []
print("\n🔍 NEXUS APEX — SMC Backtest v4 (OB zona ATR + EMA trend)")
print("="*65)
for pair in PAIRS:
    try:
...
print("="*65)
pd.DataFrame(all_results).to_csv("backtest_smc_v4_results.csv", index=False)
print("\n✅ Guardado en backtest_smc_v4_results.csv")
