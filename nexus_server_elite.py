...
def detect_patterns(df):
    ...
    if full>0 and body/full<0.1: patterns.append({"name":"DOJI","type":"neutral","desc":"Indecisin"})
    ...
    if i>0 and c[i-1]<o[i-1] and c[i]>o[i] and c[i]>o[i-1] and o[i]<c[i-1]: patterns.append({"name":"ENGULFING ALCISTA","type":"bull","desc":"Reversin al alza"})
    if i>0 and c[i-1]>o[i-1] and c[i]<o[i] and c[i]<o[i-1] and o[i]>c[i-1]: patterns.append({"name":"ENGULFING BAJISTA","type":"bear","desc":"Reversin a la baja"})
    if i>=2 and c[i-2]<o[i-2] and abs(c[i-1]-o[i-1])<(h[i-1]-l[i-1])*0.3 and c[i]>o[i] and c[i]>(o[i-2]+c[i-2])/2: patterns.append({"name":"MORNING STAR","type":"bull","desc":"Reversin alcista fuerte"})
    if i>=2 and c[i-2]>o[i-2] and abs(c[i-1]-o[i-1])<(h[i-1]-l[i-1])*0.3 and c[i]<o[i] and c[i]<(o[i-2]+c[i-2])/2: patterns.append({"name":"EVENING STAR","type":"bear","desc":"Reversin bajista fuerte"})
    if body>0 and uw<body*0.05 and lw<body*0.05: patterns.append({"name":"MARUBOZU "+"ALCISTA" if c[i]>o[i] else "BAJISTA","type":"bull" if c[i]>o[i] else "bear","desc":"Presin fuerte"})
    return patterns
...
class MLScorer:
    ...
    def __init__(self):
        # Pesos optimizados basados en backtesting histrico
        self.weights = {
            "rsi":       0.20,
            "macd":      0.18,
            "ema_cross": 0.15,
...
def calc_trailing_stop(signal, entry, current_price, atr):
    """Calcula trailing stop dinmico basado en ATR"""
    ...
    for pair in PAIRS:
        try:
            ...
            if sig in ["BUY","SELL"] and pair in ["BTCUSDT","ETHUSDT","XRPUSDT"] and sig != prev:
                ...
                msg   = tg_alert(pair, sig, conf, price, sl, tp, 2.0, trail, "Seal ML automtica", ml["ml_score"], news.get("score",0))
                send_telegram(msg)
...
def analyze_ai(pair):
    ...
    prompt = f"""Eres NEXUS PRO ELITE AI  sistema institucional de anlisis crypto con ML integrado.
Responde NICAMENTE con JSON vlido.
...
INDICADORES TCNICOS REALES:
RSI(14): {ind.get('rsi','?')} | ATR: {atr:.6f}
EMA 9: {ind.get('ema9','?')} | EMA 21: {ind.get('ema21','?')} | EMA 50: {ind.get('ema50','?')}
MACD hist: {ind.get('macd',{}).get('hist','?')} | lnea: {ind.get('macd',{}).get('line','?')}
BB posicin: {ind.get('bb',{}).get('pos','?')}% | ancho: {ind.get('bb',{}).get('width','?')}%
Estocstico K: {ind.get('stoch',{}).get('k','?')} D: {ind.get('stoch',{}).get('d','?')}
...
Seal ballenas: {ob.get('whale_signal','NEUTRAL')}
...
{{
  ...
  "strength": "FUERTE"|"MODERADO"|"DBIL",
  "reasoning": "mximo 2 oraciones en espaol",
  "key_support": nmero,
  "key_resistance": nmero,
...
if __name__ == "__main__":
    ...
    print(f"  Telegram:  {'' if 'TU_API' not in CONFIG['telegram_token'] else '  Configura token'}")
    print(""*58)
